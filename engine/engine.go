...
var egTables = board.EgTables
var mgTables = board.MgTables
...
func evalPieceSquareTables(cb *board.Board) int {
	...
	for color := range allPieces {
		for piece := range allPieces[color] {
			for allPieces[color][piece] > 0 {
				...
				if color == 0 {
					eval -= (mgPhase*mgTables[piece+1][square] +
						egPhase*egTables[piece+1][square]) / MAX_PHASE
				} else {
					eval += (mgPhase*mgTables[piece+1][square^56] +
						egPhase*egTables[piece+1][square^56]) / MAX_PHASE
				}
				allPieces[color][piece] &= allPieces[color][piece] - 1
			}
		}
	}
	eval += (mgPhase*mgTables[5][cb.KingSqs[1]^56] + egPhase*egTables[5][cb.KingSqs[1]^56]) / MAX_PHASE
	eval -= (mgPhase*mgTables[5][cb.KingSqs[0]] + egPhase*egTables[5][cb.KingSqs[0]]) / MAX_PHASE
